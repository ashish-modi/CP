...
# Time complexity : O(logN) for update and sumRange operations, O(N) for building the tree
# Space complexity : O(N) for storing the segment tree
# Explanation : A segment tree is a binary tree used for storing intervals or segments. 
# It allows for efficient querying and updating of the sum of a range of elements in an array. 
# The tree is built by recursively dividing the array into halves until we reach individual elements. 
# Each node in the tree represents the sum of a segment of the array. 
# The update operation involves updating the value of a specific index and then updating the corresponding nodes in the tree. 
# The sumRange operation involves querying the tree for the sum of a specific range by traversing the tree and summing the relevant nodes.
